...
function run_bench() {
	# java -jar $1/target/microbenchmarks.jar ".*\.(ins|pla|ent|tra).*" -rf json -rff results/$1$2.json $PARAMS | tee results/$1$2.log
	 java -jar $1/target/microbenchmarks.jar ".*Transm.*" -e ".*(Baseline|Legacy|\.packed).*" -rf json -rff results/$1$2.json $PARAMS | tee results/$1$2.log
...
function run_all() {
	...
	run_bench artemis-odb-1.4.0 _fast
